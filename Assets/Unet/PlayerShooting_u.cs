...
public class PlayerShooting_u : MonoBehaviour
{
    ...
    void Shoot() // 총 발사 
    {
        ...
        if (Physics.Raycast(shootRay, out shootHit, range, shootableMask))
        {
            ...
            if (enemyHealth != null)
            {
                // TakeDamage(입은 데미지, 맞은 지점)
                enemyHealth.TakeDamage(damagePerShoot, shootHit.point);
            }
            // 발사를 한 결과가 '참'이면 정보에 shootHit에 기록된 상태
            gunLine.SetPosition(1, shootHit.point); // 시각적 효과의 종료지점까지 지정 
        }
        else
        {
