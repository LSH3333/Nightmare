...
public class PlayerMovement_u : NetworkBehaviour
{
    ...
    [SerializeField]
    private Camera cam; // ray쏘기위한 3인칭 캠 레퍼런스 
    ...
    void FixedUpdate()
    {
        ...
        // GetAixs : -1.0f ~ 1.0f 사이의 값 리턴, 부드러운 움직임에 적합 
    ...
    private void Update()
    {
        if(cam == null)
        {
            cam = GameObject.FindGameObjectWithTag("ThirdPersonViewCamera").GetComponent<Camera>();
        }
    }
    ...
    void Move(float h, float v)
    {
        ...
        // transform.position 대신 playerRb.position으로 rb를 이용해 이동 (networktransform에서 rigidbody로 sync하기 위해서)
        playerRB.MovePosition(playerRB.position + movement * Time.fixedDeltaTime);
        */
        /*
        Vector3 _movHorizontal = transform.right * h;
        Vector3 _movVertical = transform.forward * v;
        Vector3 _velocity = (_movHorizontal + _movVertical).normalized * speed;
        if(_velocity != Vector3.zero)
        {
            playerRB.MovePosition(playerRB.position + _velocity * Time.fixedDeltaTime);
        }
        */
